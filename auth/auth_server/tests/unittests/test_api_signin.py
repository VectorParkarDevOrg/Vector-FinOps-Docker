...
class TestSignIn(TestAuthBase):
    ...
    def test_signin_keycloak_new_user(self):
        email = 'keycloak-test@domain.com'
        name = 'Keycloak User'
        roles = ['optscale-engineer']
        patch('auth.auth_server.controllers.user.UserController.'
              'domain_blacklist').start()
        token_info = (email, name, roles)
        with patch('auth.auth_server.controllers.signin.'
                   'KeycloakOauth2Provider.verify',
                   return_value=token_info):
            with patch('auth.auth_server.controllers.keycloak_role_sync.'
                       'KeycloakRoleSyncService.sync_roles'):
                code, resp = self.client.signin(
                    provider='keycloak', token='auth_code',
    ...
    def test_signin_keycloak_existing_user(self):
        patch('auth.auth_server.controllers.user.UserController.'
              'domain_blacklist').start()
        roles = ['optscale-member']
        token_info = (self.user_customer_email, self.user_customer_name, roles)
        with patch('auth.auth_server.controllers.signin.'
                   'KeycloakOauth2Provider.verify',
                   return_value=token_info):
            with patch('auth.auth_server.controllers.keycloak_role_sync.'
    ...
    def test_signin_keycloak_no_config(self):
        code, _ = self.client.post(
            self.client.signin_url(),
            {'provider': 'keycloak', 'token': 'token',
             'redirect_uri': 'https://example.com/callback'})
        self.assertEqual(code, 403)
